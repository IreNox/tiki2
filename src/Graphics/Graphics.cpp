...
{
	#pragma region Class
	Graphics::Graphics(Engine* engine, HWND hWnd)
		: hWnd(hWnd), engine(engine), inited(false), indexBuffer(0), vertexBuffers(), rasterState(0), device(0), deviceContext(0),
		  depthStencilState(0), depthStencilView(0), renderTargetView(0)
	{
	}
	...
	#pragma region Init
	bool Graphics::Initialize(const GRAPHICS_DESCRIPTION* desc)
	{
	...
	void Graphics::Clear(float* color)
	{
